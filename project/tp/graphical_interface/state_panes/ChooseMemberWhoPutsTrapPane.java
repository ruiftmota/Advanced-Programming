...
import tp.logic.state_machine.states.ChooseMemberWhoPutsTrap;
import tp.logic.ObservableGame;

public class ChooseMemberWhoPutsTrapPane extends HBox implements PropertyChangeListener
{
    ...
    public ChooseMemberWhoPutsTrapPane(ObservableGame observable_game)
    {
    ...
    private void configuration(){
        ...
        label.setText("Choose member who puts trap:");
        ...
        for(EnumCrewMembers member : EnumCrewMembers.values()){
            ...
            buttonList.get(i).setOnAction(new ChooseMemberWhoPutsTrapEvent(i+1));
            i++;
        }
    }

    class ChooseMemberWhoPutsTrapEvent implements EventHandler<ActionEvent>{
        private int id = 0;

        ChooseMemberWhoPutsTrapEvent(int id){
            this.id = id;
        }
    ...
    public void propertyChange(PropertyChangeEvent evt){
        if(observable_game.getState() instanceof ChooseMemberWhoPutsTrap == true){
            hideButtons();
            setVisible(true);
        }else{
            setVisible(false);
