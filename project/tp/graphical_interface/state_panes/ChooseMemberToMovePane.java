...
import tp.logic.state_machine.states.ChooseMemberToMove;
import tp.logic.ObservableGame;

public class ChooseMemberToMovePane extends HBox implements PropertyChangeListener
{
    ...
    public ChooseMemberToMovePane(ObservableGame observable_game)
    {
    ...
    private void configuration(){
        ...
        label.setText("Choose member to move:");
        ...
        for(EnumCrewMembers member : EnumCrewMembers.values()){
            ...
            buttonList.get(i).setOnAction(new ChooseMemberToMoveEvent(i+1));
            i++;
        }
    }

    class ChooseMemberToMoveEvent implements EventHandler<ActionEvent>{
        private int id = 0;

        ChooseMemberToMoveEvent(int id){
            this.id = id;
        }
    ...
    public void propertyChange(PropertyChangeEvent evt){
        if(observable_game.getState() instanceof ChooseMemberToMove == true){
            hideButtons();
            setVisible(true);
        }else{
            setVisible(false);
